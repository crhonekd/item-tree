...
- `streamAllStructural` uses `JdbcTemplate.query(PreparedStatementCreator, RowCallbackHandler)` (not `JdbcClient`) to set `fetchSize=5000` per-query. H2 ignores fetchSize but the code is correct for Oracle.
